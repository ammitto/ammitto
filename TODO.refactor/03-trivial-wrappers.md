...
## Defects this move carries, and must not silence

Track A moves code VERBATIM, which is what makes byte-identical output
provable. The consequence is that the relocated path keeps calling the
same collaborators, defects and all, and the move can be mistaken for a
review of them. The two below live in the models and transformers the
moved wrappers call, not in the wrapper text itself. Neither is this
card's to fix; both are this card's to name.

- **`ru` reads a shape the corpus does not have.** `transform_ru` calls
  the flat `SanctionedEntity.from_hash`, while every file in `data-ru`
  under `sources/announcements/` is the announcement schema `cn` already
  publishes from. 2,428 hand-entered entities are therefore never read.
  The fix exists as gem PR #83 and is **blocked**, not for the modelling
  but because the corpus itself is corrupt: names that are date
  fragments, organisations recorded as individuals, and 335 rows shifted
  by one. Write-up: `finding-ru-is-imported-but-unread.md` and
  `BLOCKED-ru-corpus-is-corrupt-2026-09-02.md`.
  **When this card moves `ru`, say in the PR body that the moved code is
  known not to read its own corpus.** Do not fix it here: that changes
  published output and belongs with #83.
- **`ru` and `uk` never set `circa`.** A hedged birth date publishes as
  exact. Latent for `ru` while it reads nothing; live for `uk`.
  `finding-adapters-never-set-circa-2026-09-08.md`.
