...
`## Where`, `## Done when`, `## Size and dependencies`.

An earlier draft of this convention, on the unmerged
`chore/todo-task-board` branch, also carried a `## ADHD` section per
card — a personal quick-scan summary for one reader, not a project
convention, and it duplicated `## Why this matters` / `## Where` /
`## Done when` / `## Size and dependencies` without adding information.
Dropped here and from every card in this board; that branch has since
been deleted.
...
## State on 2026-09-10, when this board was refreshed

The board was written 2026-08-21 and sat unmerged while 39 commits
landed on `main`. It was re-checked against `origin/main` on 2026-09-10,
card by card, before this refresh. **Track A has not started**, so every
card's premise is intact rather than stale:

- `lib/ammitto/cli/harmonize_command.rb` is still **1,574 lines** and
  still carries the 15-branch `case` in `transform_data`.
- `lib/ammitto/ingestion/` does not exist. No source has moved.
- All nine wrappers named in `03`, all three dispatchers in `04`, and
  `ch`, `jp` and `cn` are where the cards say they are.

What actually changed, and is corrected here:

1. **`09`'s `send` count is 49, not 51.** PR #100 folded three duplicate
   `harmonize(record)` helpers into one, taking `transform_data` from 3
   reaches to 1. No method appeared or disappeared. `13`'s ruling 5 is
   renumbered to match, since it is priced off that count.
2. **The one-class-per-file programme is effectively done**: 1 source
   file of 166 still holds more than one class, down from three source
   directories. Eight PRs landed it (#84, #95, #96, #97, #98, #99, #102,
   #105). The survivor is `un_vessels/sanctions_list.rb`, and it survives
   because nobody has ruled on whether a nested exception class counts.
   That is now **ruling 8** in `13`.
3. **Six findings logged since 2026-08-21 are in scope and were absent.**
   They are now written into the cards that move the code they affect:
   `03` (ru reads a shape its corpus does not have; ru and uk never set
   `circa`), `04` (two live AU date defects), and `07` (two live cn id
   collisions). None of them is fixed by this board. Track A moves code
   verbatim, which is what makes byte-identical output provable, so a
   defect in moved code arrives intact in its new file wearing the look
   of freshly reviewed work. Each affected card now says so, and requires
   the PR body to say so too.

The correction that mattered most is not in the list above, because it
was a claim ABOUT this board rather than in it. A handover note asserted
that these cards describe two fetch-side symbols changed by PR #68, and
that the board was therefore stale. It does not describe them, and
neither symbol appears anywhere under `TODO.refactor/` at all, which is
why this paragraph names neither. It had been
repeated onward without being checked, and it was the stated reason to
consider closing this PR.
