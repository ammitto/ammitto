# USER: eight rulings, all answered 2026-09-10

## ANSWERED 2026-09-10

All eight are ruled. Recorded here so the card stops being a question and
starts being the contract. The reasoning for each is kept below.

**1. Documentation for an output-changing refinement.** Task file, PR
body AND changelog for anything touching **exported artifacts**, each
carrying a before/after of the changed bytes. **No changelog** for
CLI-only changes that write no files: nothing downstream can observe
them.

**2. The `transform_jp` shim.** Scheduled. Track A creates it, then a
follow-up moves the four jp examples into jp's own spec file and deletes
it.

**3. How the stack lands.** Squash-merge, and do not rely on keeping the
branch. Bisectability is given up knowingly: the byte-identical proof
runs before the merge at every step, so per-commit bisect was only ever
a backstop, and a branch that any cleanup can delete is not a backstop
worth planning around.

**4. `ammitto sources`.** Preserve today's row order, derive the rows,
and land the spec first. The command has no spec at all and a
possible external JSON consumer, so its output is frozen until
something can see it change.

**5. The 49 `send` calls.** Migrate the specs to the extracted
collaborators. No delegating shims: fourteen permanent methods existing
only for tests would keep the class exactly as wide as `09` is trying to
make it narrow.

**6. D2.** Withdrawn rather than deferred, and recorded as intentional.
**0 of 3,916** AU records carry both `imo_number` and `dates_of_birth`,
and the combination is structurally unrepresentable: `Individual`
declares no `imo_number`, `Vessel` declares no `dates_of_birth`, and
each CSV row is dispatched to exactly one model before any hash
transformer sees it. `transform_from_hash` is also unreachable from the
harmonize CLI; its only caller in this repository is a spec. Revisit
only if the corpus shape changes.

**7. cn's `announcement:` key.** Do not delete the three keys yet. Fix
the feature first, then re-assess whether anything still deserves
removal. See below: the path does not merely carry dead keys, it raises.

**8. Exception classes.** Exempt from one-class-per-file. Fold
`tr/integrity_error.rb` back rather than splitting `un_vessels`.

### Ruling 7 carries a finding the original question did not know about

`Cn::Transformer#transform_modification` does not just build keys nobody
reads. It **raises** on real input. `create_official_announcement` calls
`document_id`, `type`, `signatory` and `signatory_title` on
`ModificationAnnouncementBlock`, which declares eight attributes and none
of those four. Measured:

    NoMethodError: undefined method 'document_id' for an instance of
    Ammitto::Sources::Cn::ModificationAnnouncementBlock

No spec calls `transform_modification` or `transform_cn_modification`, so
nothing has caught it. Per-file transform errors are never exempt from
the health gates, so the first real CN measure-modification file fails
the whole CN harmonize run. Fixing the feature therefore starts with a
crash, not a key rename, and the export shape has to be designed before
anything can consume what the keys hold.
...
`tr/integrity_error.rb` is the outlier, created by #105.

The wider convention agrees: thor 1.5.0 keeps all nine of its exception
classes, its base included, in one `error.rb`, moxml always groups, and lutaml-model is mixed, with 51
one-per-file in its main hierarchy but eight grouped in its xsd
subsystem. Nothing mechanical forces a choice here either, because this
gem uses explicit `require_relative` rather than an autoloader that
would demand path-to-constant agreement.
...
### 1. What documentation does an output-changing refinement owe? ANSWERED
...
### 2. Is the `transform_jp` shim permanent, or scheduled? ANSWERED
...
### 3. Does the stack squash-merge, or land as a sequence? ANSWERED
...
**Recommendation, SUPERSEDED by ruling 3, kept for the reasoning:** (b). The per-source commits are the artifact that
...
### 4. `ammitto sources` — two output changes and one definition. ANSWERED
...
### 5. How does `09` handle the 49 `send` calls? ANSWERED

Specs reach 14 private methods of `HarmonizeCommand` through `send`, 49
times (the table is in `09-REFINE-harmonize-command.md`, re-counted
2026-09-10). Extracting
...
which after Track A dispatches rather than transforms, and which three
`send` calls pin: two in `spec/integration/ingestion_robustness_spec.rb`
and one in `spec/ammitto/cli/harmonize_command_spec.rb`, re-counted
2026-09-10. Under (a) it keeps its name
and gets a comment; under (b) it is
renamed and those three migrate with the rest.

### 6. Is D2 ever scheduled, or permanently deferred? ANSWERED
...
**Recommendation, SUPERSEDED by ruling 6, which withdrew the question once the number was measured:** (b). Two dispatchers that disagree about what a
...
### 7. cn's `announcement:` key — delete, or repair? ANSWERED
...
**Recommendation, SUPERSEDED by ruling 7:** (a). Nothing downstream reads a result hash beyond
...
### 8. Does one class per file cover exception classes? ANSWERED

The programme this board tracks is effectively finished: **1 source file
of 166 still holds more than one class**, measured on `origin/main`
2026-09-10. That file is `lib/ammitto/sources/un_vessels/sanctions_list.rb`,
which nests `IntegrityError < StandardError` inside `SanctionsList`.

It is the last one because nobody has ruled on whether it counts. PR #105
split `tr`'s `IntegrityError` into `tr/integrity_error.rb` on the reading
that a class is a class. PR #102 split `un_vessels/vessel.rb` in the same
directory and left `sanctions_list.rb`'s nested error alone. So the
repository currently holds both answers, one file apart.

- **(a) Exception classes count.** Split it, matching `tr`. One S task,
  and the programme closes at 166 of 166.
- **(b) They do not.** Then `tr/integrity_error.rb` is a file that should
  not exist, and the convention needs writing down so the next split does
  not re-litigate it.

**Recommendation, SUPERSEDED by ruling 8, which reversed it on the evidence:** (a). It is smaller, it matches what already shipped in
`tr`, and "one class per file" needs no exception to stay a rule you can
check mechanically. Either way, write the answer into the convention.
...
- **Ruling 5 (b):** edits ~49 existing examples — visible churn in the
...
- 🔧 All eight answered 2026-09-10; see the block at the top. Nothing here blocks any card now.
