...
7. Add a cn ingestion unit spec (additive) — the first direct coverage
   ...
   pairs and calls `add_group` once; an unrecognised shape raises with
   the exact message.

   **The modification case cannot assert a clean nil pair, and must not
   pretend to.** `Cn::Transformer#transform_modification` RAISES on a
   real modification document: `create_official_announcement` calls
   `document_id`, `type`, `signatory` and `signatory_title` on
   `ModificationAnnouncementBlock`, which declares eight attributes and
   none of those four. Measured, `NoMethodError: undefined method
   'document_id'`. Nothing in the suite has ever called this path.

   So the spec characterises what is true today: the modification path
   raises, and the ingestion module must surface that rather than
   swallow it. A nil-pair contract test is legitimate only against a
   STUBBED transformer, and must be written as such. Repairing the
   feature is ruling 7's follow-up and belongs outside this verbatim
   move.

## Two live ID collisions in a dependency this card keeps calling

This card moves three methods OUT of `harmonize_command.rb`.
`cn/transformer.rb` does not move: the relocated ingestion path goes on
calling it. So these two are not defects this card carries into a new
file, they are defects in a collaborator the new file inherits, and the
move must not be read as having reviewed them.

Both verified by execution on `origin/main`, 2026-09-10, and they have
**different causes** despite looking alike:

- **`:201`, and this one IS the truthiness bug.**
  `name_ref = entity.english_name || entity.chinese_name`. A blank
  string is truthy in Ruby, so `||` never fires and an entity whose
  `english_name` is present but empty keeps the empty string. Two such
  entities derive the same id.
- **`:343` is NOT.** `sanitize_id(announcement_block.document_id || 'unknown')`
  looks like the same shape and is not. `sanitize_id` maps every blank
  input to the literal `unknown` itself, so the `||` is irrelevant.
  Measured:

        document_id=nil    -> .../announcement/cn/unknown
        document_id=""     -> .../announcement/cn/unknown
        document_id="   "  -> .../announcement/cn/unknown

  **The placeholder is the collision.** Any two announcements lacking a
  usable document id share one IRI, and making the `||` fire would not
  change that. The fix is not to mint an id from a placeholder at all,
  which is a design question, not a one-character correction.

PR #92 changed document-reference handling inside `Cn::Transformer`; it
did not reach either site. Write-up:
`finding-cn-blank-name-and-document-id-collision-2026-09-03.md`.

**Neither is fixed by this card.** A verbatim move is what proves the
output identical, and repairing an id here would change published ids,
which is the one thing Track A may not do. What this card owes is that
the PR body names both, so the move is not mistaken for a clearance.
