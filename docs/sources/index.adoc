...
China's corpus is curated in the `data-cn` repository from
announcement-based reference documents. Unlike `data-jp`, that repository
carries no downloading or converting tooling of its own -- its scripts
validate and merge what is already there -- so nothing automates the
step. This gem has no path to the source either: `ammitto fetch cn`
reports that and exits nonzero, and `--all` skips CN.
...
Japan data is curated in the `data-jp` repository from reference
documents covering six sanction lists:
...
asset-freeze lists, all converted with `data-jp`'s own scripts. That
tooling is the fetch: it downloads from METI and converts, and what it
produces is what `harmonize` reads. What this gem has no path to is the
same data — `ammitto fetch jp` reports that and exits nonzero, and
`--all` skips JP.
