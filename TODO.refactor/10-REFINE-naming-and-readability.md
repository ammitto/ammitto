...
delegates. It is a dispatcher. But three `send` calls pin the name
(two in `spec/integration/ingestion_robustness_spec.rb`, one in
`spec/ammitto/cli/harmonize_command_spec.rb`; that file held three
until #100 folded its duplicate helpers into one), so
...
5. Decide on `transform_data`. Either rename it to what it is
   (`dispatch`, `ingest`) and migrate the three `send` calls, or leave it
