...
Ammitto.configure do |config|
  config.api_base_url = "https://www.ammitto.org/api/v1"   # default
...
# A search that could not reach a source still returns what it could read,
# so check before treating an empty result as a clean negative
unless results.complete?
  warn "Incomplete: could not read #{results.skipped_sources.join(', ')}"
end

# Iterate results (entities)
results.each do |entity|
  puts entity.display_name
...
----
Ammitto.sources
# => [:au, :ca, :ch, :cn, :eu, :eu_vessels, :jp, :nz, :ru, :tr, :uk, :un, :un_vessels, :us, :wb]
----
...
│         │                                                                            │
│         ▼                                                                            │
│   https://www.ammitto.org/api/v1/*.jsonld                                            │
│                                                                                      │
│  ══════════════════════════════════════════════════════════════════════             │
│                                                                                      │
│  PHASE 4: GEM USERS                                                                  │
│  ══════════════════════════════════════════════════════════════════════             │
│                                                                                      │
│   Ammitto.refresh_cache → Downloads from ammitto.org/api/                       │
...
# Fetch every source with a working fetch path (skips cn, ru and jp)
...
| `api/v1/node/entity/{source}/{id}.jsonld` | Individual entity nodes (list-agnostic)
| `api/v1/node/entry/{source}/{list_type}/{id}.jsonld` | Individual entry nodes (list-specific)
...
| `api/v1/facets/*.json` | Facet counts for filters (includes list_types.json)
| `api/v1/by-list/{source}/{list_type}.jsonld` | Entries grouped by list type
...
| `AMMITTO_API_BASE_URL` | API base URL | `https://www.ammitto.org/api/v1`
