  ...
  describe 'an announcement whose parties were lost in extraction' do
    ...
    it 'refuses rather than publishing an announcement against nobody' do
      expect { transformer.transform_announcement(announcement_listing(party_list)) }
        .to raise_error(Ammitto::ParseError, /parsed no parties.*numbered list of 3\b/m)
    end

    it 'counts a list once when every translation carries it' do
      english = "1       Scott Morrison, Prime Minister\n" \
                "2       Barnaby Joyce, Deputy Prime Minister\n" \
                '3       Karen Andrews, Minister for Home Affairs'
      both = Ammitto::Sources::Ru::Announcement.from_hash(
        'announcement' => { 'document_id' => '755-07-04-2022',
                            'content' => { 'ru' => party_list, 'en' => english } },
        'sanction_details' => { 'entities' => [] }
      )

      expect { transformer.transform_announcement(both) }
        .to raise_error(Ammitto::ParseError, /numbered list of 3\b/)
    end

    it 'reads a list numbered with a full stop' do
      dotted = "1. Scott Morrison (Scott Morrison)\n2. Barnaby Joyce (Barnaby Joyce)"

      expect { transformer.transform_announcement(announcement_listing(dotted)) }
        .to raise_error(Ammitto::ParseError, /numbered list of 2\b/)
    end

    it 'reads a tab-separated list' do
      tabbed = "1\tИван Иванов\tминистр\n2\tПётр Петров\tдепутат"

      expect { transformer.transform_announcement(announcement_listing(tabbed)) }
        .to raise_error(Ammitto::ParseError, /numbered list of 2\b/)
    end

    it 'does not read out-of-order numbers as a list' do
      shuffled = "1. One\n3. Three\n2. Two"

      expect(transformer.transform_announcement(announcement_listing(shuffled))[:entities])
        .to eq([])
    end

    it 'lets a single numbered line of prose through' do
      prose = "1 The committee met (Article 5)\nNo one is named."

      expect(transformer.transform_announcement(announcement_listing(prose))[:entities])
        .to eq([])
    end
