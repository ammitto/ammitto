...
#
# data-ru does not store the flat `{ russian_name:, english_name: }` shape
# ListAnnouncement models: it stores one file per MID announcement with the
# parties named inside, `name` keyed by language, the same schema data-cn
# uses.
RSpec.describe Ammitto::Sources::Ru::Announcement do
  # Two models with incompatible schemas cannot share this name: whichever
  # file loaded last would win and leave the other unreachable.
  ...
  # Ammitto 1.0.0 published `from_parsed_data` on this name.
  describe '.from_parsed_data' do
    ...
    it 'still builds the flat announcement it built in 1.0.0' do
      built = nil
      expect { built = described_class.from_parsed_data(data) }
        .to output(/deprecated; use ListAnnouncement/).to_stderr

      expect(built).to be_a(Ammitto::Sources::Ru::ListAnnouncement)
      expect(built.entities.map(&:english_name)).to eq(['Ivanov'])
    end
  end
end
