...
| `transform_data` | 1 | `harmonize_command_spec.rb` |
...
**49 calls across 14 methods**, re-counted on `origin/main` on
2026-09-10 by grepping `send\(:<method>` in both spec files. Line numbers
are deliberately absent: they moved twice while this board sat unmerged,
and a table of stale line numbers is worse than no table.

The board first said 51, counted 2026-08-20. One method moved: PR #100
(*Derive the harmonized source from the command under test*) folded three
duplicate `harmonize(record)` helpers into one, taking `transform_data`
from 3 reaches to 1. Every other row is unchanged, and no method has
appeared or disappeared, so the extraction plan below is unaffected. The
number is recorded because the ruling in `13` is priced off it.
...
- **(b) Extract and migrate the specs.** The 49 `send` calls become
...
1. Extract input discovery to
   ...
   - `find_supplement_dirs(subdir)` iterates `@sources` in
     **requested order** and returns EVERY matching directory,
     deduplicated with `.uniq` (`harmonize_command.rb`, the
     `filter_map ... .uniq` in that method). An earlier draft of this
     card said it takes the first match; it does not, and an extraction
     written against that belief would drop supplements. Its own comment
...
4. Dispose of the code Track A leaves behind, each verify-then-remove
   ...
   - The dead `modifications:` and `announcement:` keys carried verbatim
     ...
     `legal_citations:` is not read at all. **Ruling 7 has answered
     this: do not delete them here.** The feature gets fixed first and
     the keys are re-assessed after, so this card moves them as they
     are. That is not merely deferring a tidy-up: the path RAISES on
     real input, `create_official_announcement` calling four attributes
     `ModificationAnnouncementBlock` does not declare, so there is a
     crash to fix before anything can be said about what the keys hold.
...
- `spec/ammitto/cli/harmonize_command_spec.rb` and
  `spec/integration/harmonize_pipeline_spec.rb` — the 49 `send` pins
...
- 🧨 49 spec `send` calls reach 14 private methods — extract naively and 49 examples go red; shim all 14 and the class stays wide
...
- 📦 L — 2–4 days, most of it in the 49 pins and two verify-then-removes
